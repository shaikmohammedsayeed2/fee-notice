...
const feeTotal = record => ['oldBalance', 'fees', 'books', 'others'].reduce((sum, field) => sum + (Number(record[field]) || 0), 0)
const notice = (record, compact = false) => { const total = feeTotal(record); return `<article class="notice ${compact ? 'compact' : ''}"><div class="notice-title">ANNOOR FOUNDATION SCHOOL</div><div class="identity"><div>STUDENT NAME <span>${record.studentName || '—'}</span></div><div>FATHER/GUARDIAN <span>${record.guardian || '—'}</span></div><div><b>ID NO</b> <span>${record.idNo || '—'}</span><b class="class-label">CLASS</b><span>${record.className || '—'}</span></div></div><div class="particulars">FEES - NOTICE &mdash; Particulars - Balance Amounts</div>${[['OLD BALANCE','oldBalance'],['FEES','fees'],['BOOKS','books'],['OTHERS','others']].map(([label, field]) => `<div class="fee-row"><span>${label}</span><strong>₹ ${money(record[field])}</strong></div>`).join('')}<div class="total"><span>TOTAL</span><strong>₹ ${money(total)}</strong></div><div class="notice-foot">PARENTS ARE REQUESTED TO PAY<br>THE AMOUNT BY ${paymentDate()}.</div><div class="contact">FOR DETAILS CONTACT :&nbsp; 7207506400</div></article>` }
